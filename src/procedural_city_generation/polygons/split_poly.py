...
def split_poly(poly, eps=10**-5):
    ...
    for split_edge in sorted(poly.edges, key=lambda x: -x.length):
        ...
        split_point = split_edge[0] + split_edge.dir_vector*np.random.uniform(
            0.5-singleton.split_poly_half_tolerance, 0.5+singleton.split_poly_half_tolerance)
        ...
        for other in poly.edges:
            if other is split_edge:
                ...
                new_edges[switch].append(
                    Edge(split_edge[0], split_point, split_edge.bordering_road))
                switch = not switch
                new_edges[switch].append(
                    Edge(split_point, split_edge[1], split_edge.bordering_road))
            else:
                this_cuts = False
                try:
                    x = np.linalg.solve(
                        np.array([split_edge.n, -other.dir_vector]).T, other[0] - split_point)
                ...
                if total_cuts <= 1:
                    if not this_cuts:
                        ...
                        new_edges[switch].append(
                            Edge(other[0], new_point, bordering_road=other.bordering_road))
                        new_edges[switch].append(
                            Edge(new_point, split_point, bordering_road=False))
                        switch = not switch
                        new_edges[switch].append(
                            Edge(split_point, new_point, bordering_road=False))
                        new_edges[switch].append(
                            Edge(new_point, other[1], bordering_road=other.bordering_road))
                else:
