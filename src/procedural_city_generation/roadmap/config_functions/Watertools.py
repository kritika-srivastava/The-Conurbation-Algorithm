...
class Watertools:
        ...
        while len(front) > 0:
            ...
            for x in front:
                if self.old[x[0]][x[1]] < h:
                    ...
                    new_front.extend(
                        [new for new in x+stencil if not self.flooded_tmp[new[0]][new[1]] == 1 and not np.any(
                            x <= 0) and not x[0] >= self.flooded_tmp.shape[0]-2 and not x[1] >= self.flooded_tmp.shape[1]-2]
                    )
    ...
    #                     new = [g for g in [el for el in np.array(
    #                         [x, x, x, x])-stencil] if done[g] == 0]
    #                     g[new] = 1
    #                     neufront.extend(new)
    #             front = neufront
    #             body.extend(front)
