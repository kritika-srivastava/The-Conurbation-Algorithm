...
def setup_heightmap(singleton, path):
    ...
    if name == "random":
        ...
        with open(path+"/temp/"+singleton.output_name+"_heightmap.txt", 'w') as f:
            f.write("randommap_" +
                    str(singleton.border[0])+"_"+str(singleton.border[1]))
        return 0

    # Writes correct inuse.txt
    with open(path+"/temp/"+singleton.output_name+"_heightmap.txt", 'w') as f:
        f.write(name[0:-4]+"_"+str(singleton.border[0]) +
                "_"+str(singleton.border[1]))
    ...
    indices = np.vstack(np.unravel_index(
        np.arange(array.shape[0]*array.shape[1]), array.shape)).T
