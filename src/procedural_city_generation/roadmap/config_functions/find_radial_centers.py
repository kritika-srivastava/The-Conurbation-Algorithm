...
def find_radial_centers(singleton):
    ...
    quadrate = [q for q in quadrate if np.argmax(
        img[q[0]*Aufloesung][q[1]*Aufloesung]) == 2]  # auf blaue reduzieren
    ...
    centers = [np.array([sum([x[0] for x in y])/len(y)*Aufloesung,
                         sum([x[1] for x in y])/len(y)*Aufloesung]) for y in areas]
    return centers
