...
def getRule(vertex):
    ...
    population_density = np.sqrt(
        (
            singleton.img2[int(singleton.img2.shape[0] - y * singleton.img2.shape[0])][
                int(x * singleton.img2.shape[1])
            ][0]
        )
        / 255
    )
    ...
    if not vertex.minor_road:
        ...
        intrule = np.argmax(
            singleton.img[int(singleton.img.shape[0] - y * singleton.img.shape[0])][
                int(x * singleton.img.shape[1])
            ]
        )
        z = (0, 0)
        ...
        if intrule == 2:
            z = singleton.center[
                np.argmin(np.linalg.norm(
                    vertex.coords - singleton.center, axis=1))
            ]
