...
def createtexture(name, scale):
    ...
    imagenode.image = bpy.data.images.load(
        path+"/visualization/Textures/"+name)
    ...
    mat.node_tree.links.new(
        coordsnode.outputs['Generated'], mappingnode.inputs[0])
...
def createobject(verts, faces, texname, texscale, shrinkwrap):
    ...
    if shrinkwrap:
        ...
        wrap.target = bpy.context.collection.objects['Floormesh']
...
def setupscenery():
    ...
    try:
        startup_cube = bpy.context.collection.objects.get("Cube")
        bpy.context.collection.objects.unlink(startup_cube)
    except:
        pass

    if bpy.data.objects.get("Camera") is None:
        bpy.ops.object.camera_add(view_align=True, location=(
            1.91961, -3.53902, 1.84546), rotation=(1.141, 1.56617e-08, 0.497))
...
if __name__ == '__main__':
    ...
    path = "Z:\\GithubProjects\\The-Conurbation-Algorithm\\src\\procedural_city_generation"
    import json
    global conf_values
