...
def main():
    ...
    gb = BuildingHeight(singleton.input_name,
                        singleton.buildingheight_image_name)
    ...
    for poly in polylist:
        ...
        if poly.poly_type == "road":
            ...
            floorheight = np.random.uniform(
                singleton.floorheight_min, singleton.floorheight_max)
            windowwidth = np.random.uniform(
                singleton.windowwidth_min, singleton.windowwidth_max)

            housebool = True if buildingheight < singleton.house_height else False

            if housebool:
                windowheight = np.random.uniform(
                    singleton.windowheight_min, singleton.windowheight_max_house)
                ...
                windowheight = np.random.uniform(
                    singleton.windowheight_min, windowheight_max)
                windowdist = np.random.uniform(
                    singleton.windowdist_min_not_house, windowwidth+singleton.windowdist_max_not_house)
            ...
            if housebool:
                factor = np.random.uniform(
                    singleton.scalefactor_min_house, singleton.scalefactor_max_house)
                ...
                factor = np.random.uniform(
                    singleton.scalefactor_min_not_house, singleton.scalefactor_max_not_house)
                if not poly.is_convex:
                    walls = scaletransform(walls, factor)
            ...
            if np.random.uniform(0, 1) > singleton.prob_ledge:
                ledgefactor = np.random.uniform(
                    singleton.ledgefactor_min, singleton.ledgefactor_max)
            ...
            for element in plan:
                if element == 'b' or element == 'f':
                    ...
                    if ledgefactor != 1:
                        polygons.append(buildledge(
                            ledge, currentheight, currentheight+floorheight/10, rooftexture))

                    currentheight += floorheight/10.
                elif element == 'r':

                    if ledgefactor == 1:
                        polygons.extend(roof(walls, roofwalls, currentheight, housebool,
                                             rooftexture, texGetter.getTexture("Roof", buildingheight)))

                    polygons.append(buildwalls(
                        walls, base_h_low, currentheight, walltexture))

            polygons.append(get_windows(windows, list_of_currentheights,
                                        floorheight, windowwidth, windowheight, windowdist, windowtexture))
        else:
