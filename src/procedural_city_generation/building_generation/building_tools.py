...
def buildledge(walls, bottom, top, texture):
    ...
    faces.extend([[walls.l-1, 0, walls.l, 2*walls.l-1],
                  range(walls.l), range(walls.l, 2*walls.l)])
    return Polygon3D(verts, faces, texture)
    ...
    for wall in walls.getWalls():
        ...
        if l > windowwidth:
            ...
            stencilarray = np.array(
                [stencil+np.array([0, 0, curr_h+floorheight/2]) for curr_h in list_of_currentheights])
            ...
            if n > 0:
                ...
                for i in range(n):
                    ...
                    verts.extend(np.reshape(
                        stencilarray+center_of_window, (nc*4, 3)))
            ...
            # Else build one window in the center of the wall
                ...
                verts.extend(np.reshape(
                    stencilarray+(wall[0]+(0.5*v)), (nc*4, 3)))
