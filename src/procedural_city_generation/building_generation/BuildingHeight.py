...
class BuildingHeight(object):
    ...
    def __init__(self, savename, imagename):
        ...
        try:
            with open(self.path+"/temp/"+savename + "_heightmap.txt", 'r') as f:
                self.border = [eval(x) for x in f.read().split(
                    "_")[-2:] if x is not '']
        except IOError:
        ...
        if imagename == "diffused":
            ...
            print(
                "Looking for image in procedural_city_generation/inputs/buildingheight_pictures")
            import matplotlib.image as mpimg
            self.img = mpimg.imread(
                self.path + "/inputs/buildingheight_pictures/" + imagename)
            print("Image found")
    ...
    def getBuildingHeight(self, center):
        ...
        height = self.img[int(self.img.shape[0]-y *
                              self.img.shape[0])][int(x*self.img.shape[1])][0]
        if 0 < height < 0.45:
            height = min(0.035+np.random.uniform(0, 1) *
                         np.random.uniform(0, height-0.1), 0.6)
        elif 0.45 < height:
            if height > 0.70 and np.random.uniform(0, 1) < 0.2:
                height *= np.random.uniform(1.3, 2.6)
            else:
                height = min(0.15+np.random.uniform(0, 1) *
                             np.random.uniform(0, height-0.2), 0.6)
        return height
